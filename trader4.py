...
PARAMS = {
    ...
    "basket1_strategy": {
        ...
        "history_length": 25, # Specific history length for basket 1
    },
    ...
    Product.MAGNIFICENT_MACARONS: {
        ...
        "min_profit_margin": 1,  # Minimum profit margin for arbitrage
        "position_limit": 75,
        "conversion_limit": 10,
        "sunlight_window": 5,  # Window to check sunlight trend
    },
}
...
class Trader:
    ...
    def squid_ink_fair_value(self, order_depth: OrderDepth, price_history, p = 1 , q = 1 , C1 = 0.85, C2 = 0.15, C3 = 0.05) -> float:
        ...
        def MovingAverage(price_history, q = 1):
            if len(price_history) < q + 1:
                # Not enough data to calculate ARIMA
                return 0 if price_history else None
            ...
            for i in range(1, q + 1):
                ma_terms += (price_history[-i])

            return ma_terms / q if q > 0 else 0
        ...
        if len(order_depth.sell_orders) != 0 and len(order_depth.buy_orders) != 0:
            best_ask = min(order_depth.sell_orders.keys())
            best_bid = max(order_depth.buy_orders.keys())
            return C1*((best_ask + best_bid)/2) + C2*MovingAverage(price_history, p) / 4 + C3*AutoRegress(price_history, q)
        return None
    ...
    def run(self, state: TradingState) -> Tuple[Dict[Product, List[Order]], int]:
        ...
        state.observations = state.observations.conversionObservations.get("MAGNIFICENT_MACARONS")
        ...
        if Product.SQUID_INK in self.params and Product.SQUID_INK in state.order_depths:
            ...
            movingaverage_q = self.params[Product.SQUID_INK].get("movingaverage_q", 1)
            priceWeight = self.params[Product.SQUID_INK].get("priceWeight", 1)
            MAweight= self.params[Product.SQUID_INK].get("MAweight", 1)
            ARweight = self.params[Product.SQUID_INK].get("ARweight", 1)
            ...
            if spike == "up" and squid_ink_position > -50:
                ...
                spike_trade_divisor = self.params[Product.SQUID_INK].get("spike_trade_divisor", 20)
        ...
        if Product.MAGNIFICENT_MACARONS in state.order_depths:
            ...
            conversion_obs = state.observations
            if conversion_obs is None:
                return  # Skip if no conversion observation available
            
            # Get sunlight and other parameters
            current_sunlight = conversion_obs.sunlightIndex
            critical_sunlight = PARAMS[product]['critical_sunlight_index']
            print(f"MACARONS - Current Sunlight: {current_sunlight}, Critical Sunlight: {critical_sunlight}")
            
            # Calculate sunlight trend
            sunlight_trend = 0

            if "magnificient_macarons" not in traderObject:
                traderObject["magnificient_macarons"] = {"sunlight_history": []}

            traderObject["magnificient_macarons"]["sunlight_history"].append(state.observations.sunlightIndex)
            sunlight_history = traderObject["magnificient_macarons"]["sunlight_history"]

            if len(sunlight_history) >= 2:
                sunlight_trend = sunlight_history[-1] - sunlight_history[-2]
            print(f"MACARONS - Sunlight Trend: {sunlight_trend}")
            
            # Get current position
            current_position = state.position.get(product, 0)
            print(f"MACARONS - Current Position: {current_position}")
            
            # Get order book prices
            best_ask = min(state.order_depths[Product.MAGNIFICENT_MACARONS].sell_orders.keys())
            best_bid = max(state.order_depths[Product.MAGNIFICENT_MACARONS].buy_orders.keys())
            print(f"MACARONS - Market Prices - Best Bid: {best_bid}, Best Ask: {best_ask}")
            
            # Get prices and fees from conversion observation
            obs_bid = conversion_obs.bidPrice
            obs_ask = conversion_obs.askPrice
            import_tariff = conversion_obs.importTariff
            export_tariff = conversion_obs.exportTariff
            transport_fee = conversion_obs.transportFees
            print(f"MACARONS - Conversion Prices - Bid: {obs_bid}, Ask: {obs_ask}")
            print(f"MACARONS - Fees - Import: {import_tariff}, Export: {export_tariff}, Transport: {transport_fee}")
            
            # Calculate total costs
            total_buy_cost = best_ask + import_tariff + transport_fee
            total_sell_revenue = best_bid - export_tariff - transport_fee
            print(f"MACARONS - Total Buy Cost: {total_buy_cost}, Total Sell Revenue: {total_sell_revenue}")
            
            # Check for arbitrage between order book and observations
            # Case 1: Buy from order book, sell to observations
            # CSI-based strategy (priority)
            if current_sunlight < critical_sunlight and sunlight_trend < 0:
                # Sunlight is below CSI and decreasing - enter long position
                if current_position < PARAMS[product]['position_limit']:
                    # Find best ask price
                    best_ask = min(state.order_depths[Product.MAGNIFICENT_MACARONS].sell_orders.keys()) 
                    if best_ask < float('inf'):
                        buy_quantity = PARAMS[product]['position_limit'] - current_position
                        if buy_quantity > 0:
                            if product not in result:
                                result[product] = []
                            result[product].append(Order(product, best_ask, buy_quantity))
            
            elif sunlight_trend > 0 and current_position > 0:
                # Sunlight is above CSI and increasing - exit position
                best_bid = max(state.order_depths[Product.MAGNIFICENT_MACARONS].buy_orders.keys()) 
                if best_bid > 0:
                    sell_quantity = current_position  # Apply conversion rate
                    if sell_quantity > 0:
                        if product not in result:
                            result[product] = []
                        result[product].append(Order(product, best_bid, -sell_quantity))
                        print(f"MACARONS - CSI Strategy Sell - Price: {best_bid}, Quantity: {sell_quantity}, Sunlight: {current_sunlight}")
            
            # Check for arbitrage between order book and observations (secondary priority)
            # Case 1: Buy from order book, sell to observations
            elif best_ask < obs_bid and (obs_bid - total_buy_cost) >= PARAMS[product]['min_profit_margin']:
                buy_quantity = min(
                    PARAMS[product]['position_limit'] - current_position,
                    sum(qty for _, qty in state.order_depths[Product.MAGNIFICENT_MACARONS].sell_orders.items() if _ <= best_ask)
                )
                if buy_quantity > 0:
                    if product not in result:
                        result[product] = []
            ...
            # Case 2: Buy from observations, sell to order book
            elif obs_ask < best_bid and (total_sell_revenue - obs_ask) >= PARAMS[product]['min_profit_margin']:
                sell_quantity = min(
                    current_position,
                    sum(qty for _, qty in state.order_depths[Product.MAGNIFICENT_MACARONS].buy_orders.items() if _ >= best_bid)
                )
                if sell_quantity > 0:
                    if product not in result:
                        result[product] = []
            ...
            print(f"MACARONS - End of Trading Round - Final Position: {current_position}")
